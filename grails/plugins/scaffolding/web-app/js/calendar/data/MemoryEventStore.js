...
Ext.define('Ext.calendar.data.MemoryEventStore', {
    ...
    autoSync: true,
    remoteSort: true,
    fields: ['id','cid','title','start','end'],
    proxy: {
        type: 'ajax',
        api: {
            read: window.createLink('appointment','getCalendarData')
        },
        reader: {
            type: 'json',
            root: 'data',
            getData: function(data){
                ...
                Ext.each(data.data, function(rec) {
                    rec.start =  makeDate(-20, 10);
                    rec.end = makeDate(-10, 15);
                });
                return data;
            }
        },
    ...
    listeners: {
        beforeload: function (proxy, response, operation) {
            var queryParams={
                search:"[{'key':'doctorId', 'value':'doctorID'}]"
            };
