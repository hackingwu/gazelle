...
Ext.define('Ext.calendar.App', {
    ...
    areaTree:function(){
        ...
        Ext.Ajax.request({
            async : false,
            url: window.createLink('area','getRootNode'),
            success: function(response){
                _self.areaRootData = Ext.JSON.decode(response.responseText);
                _self.areaTreeNode = _self.areaRootData.id;
            }
        });
        ...
        return Ext.create('Ext.tree.Panel', {
            store: Ext.create('Ext.data.TreeStore', {
                ...
                proxy: {
                    type: 'ajax',
                    autoLoad: true,
                    url: window.createLink('area','treeList')
                }
            }),
            width: 250,
            height: height-5,
            useArrows: true,
            root: {
                id: _self.areaRootData.id,
                text: _self.areaRootData.text,
                expanded: true
            },
            listeners:{
                itemclick: function (view, record, item, index, e, eOpts) {
                    _self.areaTreeNode =  record.data.id;
                    _self.areaTreeText =  record.data.text;
                }
            }
        });
    },
    ...
    toolBar:function(){
        var comboStore = new Ext.data.Store({
            proxy: {
                type: 'ajax',
                api: {
                    read: window.createLink('doctor','getDoctorList')
                },
                reader: {
                    type: 'json',
                    root: 'data',
...
//                    triggerAction: 'last'//,
                    ...
                    iconCls: 'icon-search',
                    handler: function () {
                        window.calendar.eventStore.loadRawData(Ext.calendar.data.Events.getData());
//                        alert("请先选择医生");
                    }
                }
            ]
