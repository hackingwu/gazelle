...
class ${className}Controller implements InitializingBean{

    static buttons = [
            //这些配在配置管理里的页面，都需要在对应的controller里去配置一个页面
            [title:"新建${domainClass.propertyName}",label:'新增',isMenu: false,parent: "${domainClass.propertyName}_index",code: "${domainClass.propertyName}_create"],
            [title:"删除${domainClass.propertyName}",label:'删除',isMenu: false,parent: "${domainClass.propertyName}_index",code: "${domainClass.propertyName}_delete"],
            [title:"更新${domainClass.propertyName}",label:'更新',isMenu: false,parent: "${domainClass.propertyName}_index",code: "${domainClass.propertyName}_update"],
            [title:"查看${domainClass.propertyName}",label:'查看',isMenu: false,parent: "${domainClass.propertyName}_index",code: "${domainClass.propertyName}_detail"]
    ]
    static allowedMethods = [save: "POST", update: "POST", delete: "POST"]

    protected Class<${className}> persistentClass = null//(Class<T>) ((ParameterizedType)(this.getClass().getGenericSuperclass())).getActualTypeArguments()[0]
...
//        e.printStackTrace()
    ...
    def list() {
        ...
        try {
            ...
            List <${className}> datas = service.list(params)
    ...
    def search(){
        ...
        List <${className}> datas =  searchResult['results'] //userSe
    ...
    def createAction() {
        ...
        service.save( persistentClass.newInstance([paramsMap] as Object[]))
     ...
     */
    def detailAction(){
        ${className} instance = service.findById(params["id"])
...
//        render output
    ...
    def treeList() {
        ...
        List<${className}> nodeDatas= persistentClass.findAllByParentId(id)
        List<Map> outputList=[];
    ...
    def getSingleSelRecord() {
        ${className} record = persistentClass.findById(2)
    ...
    def getMultiSelRecord() {
        List<${className}> datas = persistentClass.findAllById(1)
        List<Map> datasToView = [];
        if(datas.size()>=3){
            for (int k = 0; k < 3; k++) {
                datasToView << datas[k]
    ...
    void afterPropertiesSet() throws Exception {
        this.persistentClass = ${className}.class
        ...
        service = (${className}Service)grailsApplication.mainContext.getBean("\${modelName}Service")
    }
}