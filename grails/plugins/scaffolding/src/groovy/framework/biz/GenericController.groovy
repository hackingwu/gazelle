...
/**
 * Created by rabbit on 2014/8/30.
 */
abstract class GenericController<T, ID extends Serializable> implements InitializingBean{
    ...
    public GenericController(Class<T> persistentClass) {
...
//        e.printStackTrace()
    ...
    def list() {
        ...
        try {
            ...
            List <T> datas = service.list(params)
    ...
    def search(){
        ...
        List <T> datas =  searchResult['results'] //userSe
     ...
     */
    def detailAction(){
        T address = service.findById(params["id"])
    ...
    def exportData(){
        ...
        response.setHeader("Content-disposition", "attachment; filename=${modelName}.xls")

        Map domain=ModelService.GetModel(modelName)
        List fields = []
        Map labels = [:]
...
//        render output
    ...
    def treeList() {
        ...
        List<T> nodeDatas= persistentClass.findAllByParentId(id)
        List<Map> outputList=[];
    ...
    def getSingleSelRecord() {
        T record = persistentClass.findById(2)
    ...
    def getMultiSelRecord() {
        List<T> datas = persistentClass.findAllById(1)
        List<Map> datasToView = [];
        if(datas.size()>=3){
            for (int k = 0; k < 3; k++) {
                datasToView << datas[k]
    ...
    def importTpl(){
        try{
            String fileName = grailsApplication.config.cpp.membrane.erp.importTpl.get(this.modelName)
            String subPath = grailsApplication.config.cpp.membrane.erp.exportFilePath //文件上传路径
    ...
    void afterPropertiesSet() throws Exception {
        service = (GenericService)grailsApplication.mainContext.getBean(this.serviceName)
    }
