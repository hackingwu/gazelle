...
public abstract class AbstractGrailsTemplateGenerator implements GrailsTemplateGenerator, ResourceLoaderAware, PluginManagerAware {
    ...
    @Override
    public void generateService(GrailsDomainClass domainClass,Writer out) throws IOException {
        String templateText =  getTemplateText("Service.groovy");
     ...
     */
    @Override
    public void generateService(GrailsDomainClass domainClass,String destDir) throws IOException{
        Assert.hasText(destDir, "Argument [destdir] not specified");
        ...
        File destFile = new File(destDir, "grails-app/services/" + pkg.replace('.', '/') +
                domainClass.getShortName() + "Service.groovy");
        if (canWrite(destFile)) {
            destFile.getParentFile().mkdirs();
            ...
            try {
                writer = new BufferedWriter(new FileWriter(destFile));
                generateService(domainClass, writer);
                try {
                    writer.flush();
                } catch (IOException ignored) {}
            }
    ...
    @Override
    public void generateServiceTest(GrailsDomainClass domainClass,Writer out) throws IOException {
        String templateText =  getTemplateText("ServiceTest.groovy");
    ...
    @Override
    public void generateServiceTest(GrailsDomainClass domainClass, String destDir)throws IOException{
        Assert.hasText(destDir, "Argument [destdir] not specified");
        ...
        File destFile = new File(destDir, pkg.replace('.', '/') +
                domainClass.getShortName() + "ServiceTest.groovy");
        if (canWrite(destFile)) {
            destFile.getParentFile().mkdirs();
            ...
            try {
                writer = new BufferedWriter(new FileWriter(destFile));
                generateServiceTest(domainClass, writer);
                try {
                    writer.flush();
                } catch (IOException ignored) {}
            }
            ...
            log.info("ServiceTest generated at [" + destFile + "]");
        }
    }

	public void generateView(GrailsDomainClass domainClass, String viewName, Writer out) throws IOException {
        //根据domain里layout定义的属性类型，决定使用的模板
        String type = getLayoutType(domainClass) != null?getLayoutType(domainClass) :"index";
		String templateText = getTemplateText(type + ".gsp");
	...
	protected Set<String> getTemplateNames() throws IOException {
		...
		if (resourceLoader != null && grailsApplication.isWarDeployed()) {
			try {
				...
				return extractNames(resolver.getResources("/WEB-INF/templates/scaffolding/index.gsp"));
			}
			catch (Exception e) {
		...
		if (templatesDir.exists()) {
			try {
				resources.addAll(extractNames(resolver.getResources("file:" + templatesDirPath + "/index.gsp")));
			}
			catch (Exception e) {
		...
		try {
			resources.addAll(extractNames(resolver.getResources("file:" + pluginDir + "/src/templates/scaffolding/index.gsp")));
		}
		catch (Exception e) {
			// ignore
    ...
    protected Set<String> getTemplateNames(String indexName) throws IOException {
        String fileName = indexName + ".gsp";
        if (resourceLoader != null && grailsApplication.isWarDeployed()) {
            try {
                PathMatchingResourcePatternResolver resolver = new PathMatchingResourcePatternResolver(resourceLoader);
                return extractNames(resolver.getResources("/WEB-INF/templates/scaffolding/"+fileName));
            }
            catch (Exception e) {
                return Collections.emptySet();
            }
    ...
    private String getLayoutType(GrailsDomainClass domainClass){
        String typeName = null;
        try{
            GrailsClassUtils.getStaticFieldValue(domainClass.getClazz(),"m");
            Map map = (Map)GrailsClassUtils.getStaticFieldValue(domainClass.getClazz(),"m");
