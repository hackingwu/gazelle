...
class FormViewController implements InitializingBean{

    static buttons = [
            //这些配在配置管理里的页面，都需要在对应的controller里去配置一个页面
            [title:"新建formView",label:'新增',isMenu: false,parent: "formView_index",code: "formView_create"],
            [title:"删除formView",label:'删除',isMenu: false,parent: "formView_index",code: "formView_delete"],
            [title:"更新formView",label:'更新',isMenu: false,parent: "formView_index",code: "formView_update"],
            [title:"查看formView",label:'查看',isMenu: false,parent: "formView_index",code: "formView_detail"]
    ]
    static allowedMethods = [save: "POST", update: "POST", delete: "POST"]

    protected Class<FormView> persistentClass = null//(Class<T>) ((ParameterizedType)(this.getClass().getGenericSuperclass())).getActualTypeArguments()[0]
...
//        e.printStackTrace()
    ...
    def list() {
        ...
        try {
            ...
            List <FormView> datas = service.list(params)
    ...
    def search(){
        ...
        List <FormView> datas =  searchResult['results'] //userSe
     ...
     */
    def detailAction(){
        FormView instance = service.findById(params["id"])
...
//        render output
    ...
    def treeList() {
        ...
        List<FormView> nodeDatas= persistentClass.findAllByParentId(id)
        List<Map> outputList=[];
    ...
    def getSingleSelRecord() {
        FormView record = persistentClass.findById(2)
    ...
    def getMultiSelRecord() {
        List<FormView> datas = persistentClass.findAllById(1)
        List<Map> datasToView = [];
        if(datas.size()>=3){
            for (int k = 0; k < 3; k++) {
                datasToView << datas[k]
    ...
    void afterPropertiesSet() throws Exception {
        this.persistentClass = FormView.class
        ...
        serviceName = "FormViewService"

        service = (FormViewService)grailsApplication.mainContext.getBean("${modelName}Service")
    }
}