...
class FormTreeViewController implements InitializingBean{

    static buttons = [
            //这些配在配置管理里的页面，都需要在对应的controller里去配置一个页面
            [title:"新建formTreeView",label:'新增',isMenu: false,parent: "formTreeView_index",code: "formTreeView_create"],
            [title:"删除formTreeView",label:'删除',isMenu: false,parent: "formTreeView_index",code: "formTreeView_delete"],
            [title:"更新formTreeView",label:'更新',isMenu: false,parent: "formTreeView_index",code: "formTreeView_update"],
            [title:"查看formTreeView",label:'查看',isMenu: false,parent: "formTreeView_index",code: "formTreeView_detail"]
    ]
    static allowedMethods = [save: "POST", update: "POST", delete: "POST"]

    protected Class<FormTreeView> persistentClass = null//(Class<T>) ((ParameterizedType)(this.getClass().getGenericSuperclass())).getActualTypeArguments()[0]
...
//        e.printStackTrace()
    ...
    def list() {
        ...
        try {
            ...
            List <FormTreeView> datas = service.list(params)
    ...
    def search(){
        ...
        List <FormTreeView> datas =  searchResult['results'] //userSe
     ...
     */
    def detailAction(){
        FormTreeView instance = service.findById(params["id"])
...
//        render output
    ...
    def treeList() {
        ...
        List<FormTreeView> nodeDatas= persistentClass.findAllByParentId(id)
        List<Map> outputList=[];
    ...
    def getSingleSelRecord() {
        FormTreeView record = persistentClass.findById(2)
    ...
    def getMultiSelRecord() {
        List<FormTreeView> datas = persistentClass.findAllById(1)
        List<Map> datasToView = [];
        if(datas.size()>=3){
            for (int k = 0; k < 3; k++) {
                datasToView << datas[k]
    ...
    void afterPropertiesSet() throws Exception {
        this.persistentClass = FormTreeView.class
        ...
        serviceName = "FormTreeViewService"

        service = (FormTreeViewService)grailsApplication.mainContext.getBean("${modelName}Service")
    }
}