...
class OrganizationController implements InitializingBean{

    static buttons = [
            //这些配在配置管理里的页面，都需要在对应的controller里去配置一个页面
            [title:"新建organization",label:'新增',isMenu: false,parent: "organization_index",code: "organization_create"],
            [title:"删除organization",label:'删除',isMenu: false,parent: "organization_index",code: "organization_delete"],
            [title:"更新organization",label:'更新',isMenu: false,parent: "organization_index",code: "organization_update"],
            [title:"查看organization",label:'查看',isMenu: false,parent: "organization_index",code: "organization_detail"]
    ]
    static allowedMethods = [save: "POST", update: "POST", delete: "POST"]

    protected Class<Organization> persistentClass = null//(Class<T>) ((ParameterizedType)(this.getClass().getGenericSuperclass())).getActualTypeArguments()[0]
...
//        e.printStackTrace()
    ...
    def list() {
        ...
        try {
            ...
            List <Organization> datas = service.list(params)
    ...
    def search(){
        ...
        List <Organization> datas =  searchResult['results'] //userSe
     ...
     */
    def detailAction(){
        Organization instance = service.findById(params["id"])
...
//        render output
    ...
    def treeList() {
        ...
        List<Organization> nodeDatas= persistentClass.findAllByParentId(id)
        List<Map> outputList=[];
    ...
    def getSingleSelRecord() {
        Organization record = persistentClass.findById(2)
    ...
    def getMultiSelRecord() {
        List<Organization> datas = persistentClass.findAllById(1)
        List<Map> datasToView = [];
        if(datas.size()>=3){
            for (int k = 0; k < 3; k++) {
                datasToView << datas[k]
    ...
    void afterPropertiesSet() throws Exception {
        this.persistentClass = Organization.class
        ...
        service = (OrganizationService)grailsApplication.mainContext.getBean("${modelName}Service")
    }
}