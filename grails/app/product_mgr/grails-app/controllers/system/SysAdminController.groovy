...
class SysAdminController implements InitializingBean{

    static buttons = [
            //这些配在配置管理里的页面，都需要在对应的controller里去配置一个页面
            [title:"新建sysAdmin",label:'新增',isMenu: false,parent: "sysAdmin_index",code: "sysAdmin_create"],
            [title:"删除sysAdmin",label:'删除',isMenu: false,parent: "sysAdmin_index",code: "sysAdmin_delete"],
            [title:"更新sysAdmin",label:'更新',isMenu: false,parent: "sysAdmin_index",code: "sysAdmin_update"],
            [title:"查看sysAdmin",label:'查看',isMenu: false,parent: "sysAdmin_index",code: "sysAdmin_detail"]
    ]
    static allowedMethods = [save: "POST", update: "POST", delete: "POST"]

    protected Class<SysAdmin> persistentClass = null//(Class<T>) ((ParameterizedType)(this.getClass().getGenericSuperclass())).getActualTypeArguments()[0]
...
//        e.printStackTrace()
    ...
    def list() {
        ...
        try {
            ...
            List <SysAdmin> datas = service.list(params)
    ...
    def search(){
        ...
        List <SysAdmin> datas =  searchResult['results'] //userSe
     ...
     */
    def detailAction(){
        SysAdmin instance = service.findById(params["id"])
...
//        render output
    ...
    def treeList() {
        ...
        List<SysAdmin> nodeDatas= persistentClass.findAllByParentId(id)
        List<Map> outputList=[];
    ...
    def getSingleSelRecord() {
        SysAdmin record = persistentClass.findById(2)
    ...
    def getMultiSelRecord() {
        List<SysAdmin> datas = persistentClass.findAllById(1)
        List<Map> datasToView = [];
        if(datas.size()>=3){
            for (int k = 0; k < 3; k++) {
                datasToView << datas[k]
    ...
    void afterPropertiesSet() throws Exception {
        this.persistentClass = SysAdmin.class
        ...
        serviceName = "SysAdminService"

        service = (SysAdminService)grailsApplication.mainContext.getBean("${modelName}Service")
    }
}